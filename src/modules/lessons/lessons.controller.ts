...
import { ApiBody, ApiCookieAuth, ApiOperation, ApiParam, ApiQuery } from '@nestjs/swagger';
...
export class LessonsController {
    ...
    @Roles(Role.SUPERADMIN, Role.CREATOR, Role.ADMIN, Role.TEACHER)
    @ApiOperation({
        summary: "Guruhning ma'lum bir yil, oy yoki kunga tegishli o'tilgan darslari",
        description: "Guruhning faqat o'tilgan (conducted) darslarini yil, oy va kun bo'yicha filterlab qaytaradi.\n\n" +
                     "**Ruxsat (Access):** Rollar: `SUPERADMIN`, `CREATOR`, `ADMIN`, `TEACHER`"
    })
    @ApiQuery({ name: 'groupId', required: true, type: Number, example: 1, description: "Guruh ID raqami" })
    @ApiQuery({ name: 'year', required: false, type: Number, example: 2026, description: "Yil (masalan: 2026)" })
    @ApiQuery({ name: 'month', required: false, type: Number, example: 5, description: "Oy (1-12)" })
    @ApiQuery({ name: 'day', required: false, type: Number, example: 17, description: "Kun (1-31)" })
    getLessonsByDate(
